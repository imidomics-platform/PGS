...
# Project settings ----
proj_dir <- "/Projects/Internal_Projects/2021_IMX_TargetIdentification_vMR3/01_MR_pQTL/"

# Shared utilities + config (same pattern as script 01)
    ...
    if (!is.null(res) && nrow(res) > 0) {
      ...
      if (nrow(res) > 0) {
        ...
        if (sum(ins2$rsid %in% eur.bim$V2) > 1 && any(ins2$PAV)) {
          ins_ld <- ld_matrix(variants = ins2$rsid,
                              plink_bin = plink_bin,
