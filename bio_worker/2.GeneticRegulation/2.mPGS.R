...
if (tissue=="skin" | tissue=="salivary_gland") {
  df.mol.samples<-merge(dataset_samples_mol[dataset_samples_mol$timepoint=="wk0" & dataset_samples_mol$tissue_type=="affected" & dataset_samples_mol$disease==disease_of_interest,c("sex","age","timepoint","sample_id","followup_id")], dataset_tech_mol[,c("sample_id","Sequencing_batch")], by="sample_id")
} else {
  df.mol.samples<-merge(dataset_samples_mol[dataset_samples_mol$timepoint=="wk0" & dataset_samples_mol$disease==disease_of_interest,c("sex","age","timepoint","sample_id","followup_id")], dataset_tech_mol[,c("sample_id","Sequencing_batch")], by="sample_id")
...
saveRDS(res,paste0(tmp.folder,"/",disease_of_interest,"_PGS_",assay2,"_",tissue,".rds"))
dim(res)
