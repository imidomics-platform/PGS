...
analysis_covs <-c("sex", "age", "eGFR","Dim.1")                   # 5 IMIDs: c("sex","age","Dim.1","Dim.2","Dim.3","Dim.4"); SLE: c("sex","age","Dim.1","Dim.2","PleateId"); SSAD: c("sex", "age", "eGFR","Dim.1")
...
covs2a<-colnames(df.prot)[colnames(df.prot) %in% analysis_covs]
...
if (analysis_method=="LogisticRegression") {
  ...
  results <- lapply(mol2analyze, function(mol) {
    formula <- as.formula(paste0("IMID ~ ","`", mol, "` + ",covs.text))
...
if (analysis_method=="LimmaLinearModel") {
  ...
  meta <- df.prot[, c("disease", analysis_covs)]
  meta$disease <- factor(meta$disease)
  meta$sex <- factor(meta$sex)
  
  if ("PlateId" %in% colnames(meta)) {
    metalPateId <- factor(meta$PlateId)
  ...
  expr <- as.matrix(df.prot[, !(colnames(df.prot) %in% c("Row.names", "disease", analysis_covs))])
  expr <- t(expr)
  ...
  res$TechID <- rownames(res)
  res<-merge(res,corresp,by="TechID", all.x=T)
  res<-res[,c("Ensembl_ID","TechID","Symbol","logFC","P.Value")]
  colnames(res)<-c("EnsemblID","TechID","Symbol","Effect","Pvalue")
